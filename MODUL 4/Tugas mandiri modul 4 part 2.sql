...
-- Tugas Latihan mandiri Modul 4 part 2
-- latihan 1
...
SELECT nama_produk, SUM(qty) AS total_qty_tr_terjual
FROM tr_penjualan
GROUP BY nama_produk
HAVING SUM(qty) = 9;
...
-- (harga dikalikan jumlah dikurangi diskon) dengan urutan berdasarkan nilai penjualan terbesar
SELECT nama_produk, SUM(harga * qty * (1 - diskon_persen / 100)) AS total_nilai_tr_penjualan
FROM tr_penjualan
GROUP BY nama_produk
ORDER BY total_nilai_tr_penjualan DESC;
